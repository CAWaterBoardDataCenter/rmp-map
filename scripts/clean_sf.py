"""SF Bay RMP: sf_bay_raw.tsv (tab-separated) → data/cleaned/sf_bay.csv

Pseudocode:
  LOAD tab-separated sf_bay_raw.tsv (CD3 analyte export; many rows per station)
...
PROGRAM_ID = "sf_bay"
RAW = ROOT / "data" / "raw" / "sf_bay_raw.tsv"
...
def main() -> None:
    if not RAW.exists():
        raise FileNotFoundError(f"Place SF Bay raw export at {RAW.relative_to(ROOT)}")
    ...
    print(f"SF Bay: {len(stations)} stations -> {OUTPUT.relative_to(ROOT)}")
